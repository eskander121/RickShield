...
def generate(frames, text_lines):
	...
	if not os.path.exists("/home/eskander/snapshots"):
		os.makedirs("/home/eskander/snapshots")

	# Generate a filename based on the current time
	filename = datetime.datetime.utcnow().strftime("%Y%m%dT%H%M%S.jpg")
	# Write the image to that file
	cv2.imwrite("/home/eskander/snapshots/" + filename, snap)

	# Return the saved file location
	return "/home/eskander/snapshots/" + filename

	# Made sure a snapshot folder exist
	if not os.path.exists("/run/user/1000/gvfs/ftp:host=192.168.2.8,port=12345/storage/emulated/0/Download/snapshots"):
		os.makedirs("/run/user/1000/gvfs/ftp:host=192.168.2.8,port=12345/storage/emulated/0/Download/snapshots")

	# Generate a filename based on the current time
	filename = datetime.datetime.utcnow().strftime("%Y%m%dT%H%M%S.jpg")
	# Write the image to that file
	cv2.imwrite("/run/user/1000/gvfs/ftp:host=192.168.2.8,port=12345/storage/emulated/0/Download/snapshots/" + filename, snap)

	# Return the saved file location
	return "/run/user/1000/gvfs/ftp:host=192.168.2.8,port=12345/storage/emulated/0/Download/snapshots/" + filename
